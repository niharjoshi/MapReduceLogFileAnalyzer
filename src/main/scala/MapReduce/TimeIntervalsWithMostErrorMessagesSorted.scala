...
object TimeIntervalsWithMostErrorMessagesSorted:
  ...
  class TokenizerMapper extends Mapper[Object, Text, Text, IntWritable] {
    ...
    override def map(key: Object, value: Text, context: Mapper[Object, Text, Text, IntWritable]#Context): Unit = {
      ...
      // Processing only log messages of type input
      if (logType == "ERROR") {

        // Creating the key for our mapper - timeinterval
  ...
  class IntSumReader extends Reducer[Text, IntWritable, Text, IntWritable] {
    ...
    // This is our reducer
    // It takes the mapper's output and counts the number of error log messages for each time interval
    override def reduce(key: Text, values: lang.Iterable[IntWritable], context: Reducer[Text, IntWritable, Text, IntWritable]#Context): Unit = {

      // Counting the total number of error log messages for each time interval
